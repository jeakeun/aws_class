...
# 학생이 등록된 학년과 반을 조회
select distinct grade 학년, class 반 from student;
...
# 집계함수
# count(속성명) : 개수
# count(*)인 경우 모두가 null이 아니면 개수를 셈.
# count(속성명)인 경우 컬럼(속성명)이 null이면 개수를 안셈
# sum(속성명) : 합
# avg(속성명) : 평균
# min(속성명) : 최소값
# max(속성명) : 최대값
...
# 각 학년 학기별 평균이 90이상인 과목들을 조회하는 쿼리
select sj_code 과목번호, avg(score) 평균 from score 
# where avg(score) >= 90 실행 시 에러발생. 집계함수는 where절에서 사용 불가
group by sj_code
having 평균 >= 90;  # 평균대신 avg(score)도 가능

/*
select 컬럼1(속성명),컬럼2(속성명), ...
from 테이블명
where 조건
group by 컬럼명1(속성명1), ...
having 조건
oreder by 컬럼명1(속성명) 정렬방법, ...
limit 시작번지, 개수
...
from score 
join subject on score.sj_code = student.code
group by sj_code;
...
# 두 테이블을 연결한 후 연결할 데이터가 없는 쪽은 null 처리
...
# right join : 테이블1을 기준으로 테이블2 옆에 테이블1를 이어 붙임
# select * from 테이블1
...
# 학생의 성적을 조회(학년, 반, 번호, 이름, 학년, 학기, 과목, 성적)
select 
...
# - having절에서 특정 값 대신 사영
# - insert문에서 value 대신에 사용. insert select문
