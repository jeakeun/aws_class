...
# 이순신 회원이 헬스장에 가입했습니다. 관리자는 A구역 1번 사물함을 배정
# 연락처 : 020-1111-1111, 가입일은 현재시간
# 기본값 CURRENT)TIMESTAMP
# - INSERT 될 때 값을 넣지 않으면 기본값으로 현재 시간의 년,월,일,시,분,초가 자동으로 추가
# - 단, DATETIME에서만 가능
INSERT INTO MEMBER(NAME,CONTACT,LOCKER_ID)
	VALUES('이순신','020-1111-1111',1),
		  ('성춘향','020-2222-2222',6);
# 성춘향 회원이 헬스장에 가입했습니다. 관리자는 B구역 1번 사물함을 배정
# 연락처 : 020-2222-2222, 가입일은 현재시간
# INSERT INTO MEMBER(NAME,CONTACT,JOIN_DATE,LOCKER_ID)
#	SELECT '성춘향','020-2222-2222', NOW(),LOCKER_ID
#		FROM LOCKER
#        # B구역 1번 사물함을 B구역-01으로 변환하여 검색 후 추가
#  LPAD(1, 2, '0') < 두자리 숫자로 정하는데 두자리 숫자가 안되면 첫번째 자리를 0으로 만듬
...
# 이순신(1) 회원이 지옥의 크로스핏 강좌를 출석체크 함(현재시간)
