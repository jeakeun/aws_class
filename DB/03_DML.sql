...
# insert into 테이블명(컬럼1, 컬럼2, ..., 컴럼N) values(값1,값2,...,값N)
...
#   where
#       조건
# SQL에서 =은 대입연사자가 아니라 비교 연산자 == 의미
# =이 대입연산자 역할을 하려면 앞에 set이 나와야 함
# 같다 : =; 같지 않다 : != 또는 <>
# where이 없으면 모든 행을 수정
# 자바에서 &&연산자가 MySQL에서는 AND, ||연산자가 OR
# 자바에서는 &&와 || 연산자 우선순위가 같지만
# MySQL에서는 AND가 OR보다 높음

# 아래 코드 실행시 에러가 발생(워크벤치에서)
# 워크벤치에서는 기본키를 이용하지 않은 수정이나 삭제를 방치(기본값)
# 설정 변경 방법
# Edit > preferences... > SQL Editor에서 제일 하단에 Safe Update체크박스 해제
...
insert into score(st_code, sj_code, score)
value(6, 1, 100), (6, 3, 100);
# 1학년 2반 1번 학생의 1학년 1학기 성적 - 국어 : 50, 수학 : 100
insert into score(st_code, sj_code, score)
value(7, 1, 50), (7, 3, 100);
