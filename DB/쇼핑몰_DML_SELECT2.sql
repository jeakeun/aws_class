...
# abcc123회원의 장바구니 목록 조회

# select * from 테이블1 join 테이블2 on 테이블1.컬럼1 = 테이블2.컬럼2; 컬럼두개의 이름이 같을떄 using(컬럼)사용가능
select * from cart join product on cart.code = product.code
where id = "abc123";
# abc123회원이 구매한 구매 내역 조회
select * from buy join product 
on buy.code = product.code
where id = "abc123";

# abc123회원의 누적 금액을 조회
select id, ifnull (sum(price * buy.amount),0) 누적금액 from buy
