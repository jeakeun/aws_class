...
# 모닝 요가를 수강신청한 인원을 조회
# 방법1 : 모닝 요가의 키값을 알 때
SELECT COUNT(CLASS_ID) FROM ENROLLMENT 
WHERE CLASS_ID = 1;

# 방법2 : 모닝 요가의 키값을 모를 때, 서브 쿼리
# SELECT COUNT(CLASS_ID) FROM ENROLLMENT 
# WHERE CLASS_ID = (SELECT CLASS_ID FROM CLASS WHERE NAME='모닝요가');
...
FROM (SELECT * FROM ATTENDANCE
	  WHERE 
		DATE(CHECK_IN_TIME) = DATE(NOW())) ATT
	RIGHT JOIN CLASS USING(CLASS_ID)
GROUP BY CLASS.CLASS_ID;
