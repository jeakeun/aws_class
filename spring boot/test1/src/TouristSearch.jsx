...
const TouristList = () => {
  ...
  const fetchBigData = () => {
    ...
    // ✅ 중요: 주소를 /api/tour/stats 로 변경했습니다.
    // 날짜는 데이터가 확실히 있는 2024년 1월 1일로 테스트합니다.
    const url = `http://localhost:8080/api/tour/stats?areaCode=${areaCode}&startYmd=20240101&endYmd=20240101`;
    ...
    fetch(url)
      ...
      .then(data => {
        setItems(data);
        setLoading(false);
        if (data.length === 0) setErrorMsg("해당 지역에 대한 데이터가 응답에 없습니다.");
      })
      .catch(err => {
        console.error(err);
        setErrorMsg("서버 연결 실패! 스프링 부트의 @CrossOrigin과 주소를 확인하세요.");
        setLoading(false);
      });
  };
